...
for (const name of packages) test(`package ${name} imports and stubs never fake success`, () => {
  ...
  if (!['core', 'cli'].includes(name)) for (const fn of Object.values(api)) assert.throws(() => fn({}), NotImplementedStageError);
});
test('core verdict resolution is also explicitly unimplemented', () => {
  const core = require('@isotope/core');
  assert.throws(() => core.resolveVerdict({}), NotImplementedStageError);
  assert.throws(() => core.resolveAggregateVerdict([]), NotImplementedStageError);
});
test('package graph is acyclic and core is the only subsystem dependency', () => {
  ...
  function visit(p) {
    ...
    for (const dependency of Object.keys(manifests[p].dependencies || {})) if (dependency.startsWith('@isotope/')) {
      assert.notEqual(p, 'core'); assert.equal(dependency, '@isotope/core'); visit(dependency.slice(9));
    }
...
const commands = [['scan'], ['verify'], ['verify','--no-reasoner'], ['verify','--no-repair'], ['repair','ep-test'], ['repair','--explain','r1'], ['explain','ep-test'], ['fleet','--repos','corpus/repos.json','--spec','test','--out','dashboard.html','--reason','--repair'], ['spec','validate'], ['spec','draft','--url','https://example.invalid','--provider','test'], ['spec','list'], ['fixtures','normalize'], ['matrix'], ['accuracy']];
...
test('product fixture directories contain no invented payloads', () => {
  for (const directory of ['fixtures/raw', 'fixtures/normalized', 'corpus']) assert.deepEqual(readdirSync(path.join(root,directory)), ['README.md']);
});
