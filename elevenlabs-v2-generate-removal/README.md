...
This standalone benchmark captures the documented ElevenLabs Python SDK migration from `elevenlabs==1.59.0` to `elevenlabs==2.0.0`. It does not require Isotope and does not make live requests.
...
.venv-old/bin/pip install -r requirements-old.txt
python3 -m venv .venv-new
.venv-new/bin/pip install -r requirements-new.txt
