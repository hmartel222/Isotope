...
- `isotope-demo-elevenlabs-removal`: a dependency-only ElevenLabs Python 1.59.0 → 2.x update removes `ElevenLabs.generate`, drops the storage sink, and produces a mechanical `FAIL` followed by a Gemini-planned repair that is independently verified against planning and held-out fixtures.
