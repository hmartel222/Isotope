...
- Stripe's dedicated strategy supports default/named `Stripe` construction, sync/async `webhooks.constructEvent`, and configured `subscriptions.retrieve` (`http_out`). Webhook calls return a fresh fixture clone. Webhook-style plans with a provider mock must actually invoke interception; installing the mock alone is insufficient. Signature verification errors invalidate execution even if both payloads fail alike. Plain plans can opt into the same assertion.
