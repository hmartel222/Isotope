...
export async function checkResolverCompatibility(input: ResolverCompatibilityInput): Promise<ResolverCompatibilityResult> {
  ...
  const packages = input.candidateSpec.detection.ecosystems.npm?.packages ?? [];
  const matching = modules.filter(module => packages.some(pkg => module === pkg || module.startsWith(`${pkg}/`)));
  if (new Set(matching).size > 1) return { status: 'ambiguous', language: 'ts', module: matching.sort().join(','), matchedRoots: [], diagnostics: [`Multiple configured provider modules match: ${matching.join(', ')}`] };
  try {
    ...
    return { status: /unsupported|pattern|path/i.test((error as Error).message) ? 'unsupported' : 'unbound', language: 'ts', module: matching[0] ?? packages[0] ?? 'unknown', matchedRoots: [], diagnostics: [(error as Error).message] };
  }
}
