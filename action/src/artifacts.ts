...
import { readJsonArtifact, type BDG, type CandidatePatch, type DiffReport, type IsotopeReport, type ReasoningResult, type SelectedSpecs, type Signature } from '@isotope/core';
...
export async function loadReportEvidence(rootInput: string): Promise<ReportEvidence> {
  ...
  const candidates: CandidatePatch[] = []; for (const ref of report.candidateRefs) candidates.push(await readJsonArtifact(root, join(root, ref), 'CandidatePatch'));
