...
The Action supports `reasoner: on|off` and `repair: on|off`. Repair runs only deterministic ChangeSpec codemods in an isolated worktree and keeps a verified-repair check red because the underlying product verdict remains `FAIL`. Semantic reasoning uses a bounded evidence packet and two independent votes; it cannot override a mechanical `FAIL`. The action never installs customer dependencies and has no commit, push, merge, or PR-creation capability.
